<?php
    ...
    $tutorSQL = "SELECT * FROM tutor_applications WHERE Student_ID = $user_id";
    $tutorResult = $conn->query($tutorSQL);
     
    if ($tutorResult->num_rows > 0) {
        $row = $tutorResult->fetch_assoc();
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($new_role == 'tutor') {
            // Check if the student ID already exists in the tutor table
            $tutorCheckQuery = "SELECT * FROM tutor_applications WHERE Student_ID = ?";
            $tutorStmt = $conn->prepare($tutorCheckQuery);
            $tutorStmt->bind_param("s", $user_id);
            $tutorStmt->execute();
            ...
            if ($tutorResult->num_rows > 0) {
                ...
                $tutorUpdateQuery = "UPDATE tutor_applications SET Course = ?, Major = ?, Year = ?, Semester = ?, Concentration = ?, Role = ?, Date_updated = ? WHERE Student_ID = ?";
                $tutorStmt = $conn->prepare($tutorUpdateQuery);
                if($tuteeStmt){
                        $tutorStmt->bind_param("ssssssss", $new_course, $new_major, $new_year, $new_semester, $new_concentration, $new_role, $date_updated, $user_id);
        ...
        elseif ($new_role == 'tutee') {
            $tuteeInsertQuery = "INSERT INTO tutee_applications (Student_ID, Full_Name, Course, Major, Year, Semester, Concentration, Role, Date, Date_updated) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $tuteeStmt = $conn->prepare($tuteeInsertQuery);
            $tuteeStmt->bind_param("ssssssssss", $user_id, $fullName, $new_course, $new_major, $new_year, $new_semester, $new_concentration, $new_role, $date, $date_updated);
...
<body>
    <form action="edit_tutor_application.php">
